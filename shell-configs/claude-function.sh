# claude-function.sh — Worktree-first shell function for Claude Code.
#
# This file defines a `claude()` shell function that prevents Claude Code
# sessions from accidentally working on the default branch (main/master)
# of a git repo. When it detects that situation, it offers to create or
# select a worktree before launching Claude inside it.
#
